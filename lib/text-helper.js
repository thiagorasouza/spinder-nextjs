export default class TextHelper {
  ...
  extractSummary({ maxChars, maxSentences, nextSectionDelimiter }) {
    this.stripAfterDelimiter(nextSectionDelimiter);
  ...
  stripAfterDelimiter(delimiter) {
    const delimiterIndex = this._text.search(delimiter);
    if (delimiterIndex !== -1) {
